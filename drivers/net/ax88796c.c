		...
		unsigned int regno)
{
	...
	data = *((volatile unsigned char *)dev->iobase + regno);
	data |= (*((volatile unsigned char *)dev->iobase + regno + 1) << 8);
...
static inline void put_reg16 (struct eth_device *dev, unsigned int regno, 
		unsigned short val)
{
	*((volatile unsigned char *) dev->iobase + regno) = (val & 0x00FF);
	*((volatile unsigned char *) dev->iobase + regno + 1) = (val >> 8);
}
#endif
...
{
	unsigned short rcphr, rxlen, i, burst_len; // pio_len - Fix unused
	unsigned char *addr;
	unsigned long time_out;
		...
		if (ax_local->w_state == chk_cable) {
			...
			if (++ax_local->w_ticks == AX88796C_WATCHDOG_RESTART) {
				ax88796c_mdio_write (dev, PHY_ID, PHY_BMCR,
						PHY_BMCR_100MB | 
						PHY_BMCR_AUTON | 
						PHY_BMCR_RST_NEG|PHY_BMCR_DPLX);
...
static int ax88796c_recv (struct eth_device *dev)
{
	...
	return 0; // FixMe: only one return point???
}
