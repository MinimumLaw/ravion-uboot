...
static int mfgr_fuse(void)
{
	...
	fuse_sense(0, 5, &val);
	printf("Fuse 0, 5 (OCOTP_CFG4, addr 5, BOOT_CFG[4..1]): %8x\n", val);
	fuse_sense(0, 6, &val6);
	printf("Fuse 0, 6 (OCOTP_CFG5, addr 6): %8x\n", val6);
	fuse_sense(4, 2, &val);
	printf("Fuse 4, 2 (OCOTP_MAC0, addr 22): %8x\n", val);
	fuse_sense(4, 3, &val);
	printf("Fuse 4, 3 (OCOTP_MAC1, addr 23): %8x\n", val);
	if (val6 & 0x10) {
		puts("BT_FUSE_SEL already fused, will do nothing\n");
//		return CMD_RET_FAILURE;
	}
	/* boot cfg */
	// BOOT_CFG1 = 0x62 (MME/eMMC, regular, eMMC reset enabled)
	// BOOT_CFG2 = 0x50 (8 bit, uSDHC3)
	// BOOT_CFG3 = 0x30 (DDR ext, Single channel, NOC disabled, MMDC reorder enabled, ARM - 792, DDR - 528)
	// BOOT_CFG4 = 0x00 (ECSPI1, SS0, 2-byte (16 bit) addressing)
	// fuse_prog(0, 5, 0x00305062);
	// Toradex - BOOT_CFG3 00 - DDR Single, NOC enabled, MMDC reorder disabled, 792/528
